...
export default function Hero() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentMessage((prev) => (prev + 1) % ecoMessages.length);
    }, 4000);
  ...
  return (
    <>
      ...
      <div className="w-full bg-gradient-to-r from-fern/10 via-sage/10 to-fern/10 border-b border-fern/20 py-4 overflow-hidden">
        <div className="flex items-center justify-center relative h-12">
          {ecoMessages.map((msg, idx) => (
            <div
              key={idx}
              className={`absolute transition-all duration-1000 transform ${
                idx === currentMessage
                  ? 'opacity-100 translate-y-0'
                  : idx === (currentMessage - 1 + ecoMessages.length) % ecoMessages.length
                  ? 'opacity-0 -translate-y-4'
                  : 'opacity-0 translate-y-4'
              }`}
            >
              <span className="text-lg font-semibold text-ink font-jost tracking-wide">
                {msg}
              </span>
            </div>
          ))}
